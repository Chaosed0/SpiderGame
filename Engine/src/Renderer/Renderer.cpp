...
void Renderer::drawInternal(RenderSpace space)
{
	...
	for (auto iter = entityPool.begin(); iter != entityPool.end(); iter++) {
		...
		GLenum drawType;
		if (model.material.drawType == MaterialDrawType_Lines) {
			drawType = GL_LINES;
		} else if (model.material.drawType == MaterialDrawType_Triangles) {
			drawType = GL_TRIANGLES;
		}

		const Mesh& mesh = model.mesh;
		glBindVertexArray(mesh.impl->VAO);
		glDrawElements(drawType, mesh.impl->nIndices, GL_UNSIGNED_INT, 0);
		glBindVertexArray(0);
		glCheckError();
	}
}
